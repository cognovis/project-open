...
ad_proc -public im_dynfield::append_attributes_to_form {
    ...
    <ul>
     <li>absolute: we just wrap the label-widget attribute in div's using one *-absolute adp file.
                   It is up to the css to place these attributes.
     <li>relative: we sort the attributes and display them using a *-relative adp file.
     <li>adp: There is an specific adp_file to format this form, use it.
    </ul></p>

    @option object_type     The object_type attributes you want to add to the form

    @option style_var       The varname from the caller program which contains the adp file you want to use for the 
    ...
    #-------------------------------------------
    #  add object_type and object_id vars to form
    #-------------------------------------------
    ...
    #-------------------------------------------
    #  only in edit mode its necessary object_id
    #-------------------------------------------
	...
	#ns_log notice "$attribute_name ------> datatype $datatype transdatatype $translated_datatype widget $widget"
	# ------------------------------------------------
	...
	#append html_parameters " id $attribute_name"
	...
	# ------------------------------------------------
	#ns_log notice "$attribute_name ------> datatype $datatype widget $widget"
	...
	if {[info exists object_id]} {
		...
		where \
			object_type = :object_type
    	     ...
    	     # ------------------------------------------------
    	     # get extension tables for object_type
    	     # ------------------------------------------------
		...
		# ------------------------------------------------
		set sql "select attribute_id, attribute_name 
			 from acs_attributes 
			 where object_type = :object_type"
			 
		if {$t_name != $table_name} {
			append sql " and table_name = :t_name"
		} else {
		
			# ------------------------------------------------
			# attributes in main object_type table 
			# have empty table_name value in acs_attributes
			# ------------------------------------------------
		
			append sql " and table_name is null"
			
		}
		set attributes_table_list_of_lists [db_list_of_lists "get attributes" "$sql"]

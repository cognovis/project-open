...
ad_proc -public im_timesheet2_sync_timesheet_costs {
    ...
    # Select all timesheet hours, that dont have a financial item associated.
    set hour_user_constraint ""
    set costs_user_constraint ""
    if {0 != $user_id} {
	set hour_user_constraint "and h.user_id = :user_id"
	set costs_user_constraint "and c.cause_object_id = :user_id"
    }

    set invoicable_hours_sql "
	select
		h.day,
		h.project_id,
		h.user_id
	from
		im_hours h
	where	1=1
		$hour_user_constraint
    EXCEPT
	select
		c.effective_date as day,
		c.project_id,
		c.cause_object_id as user_id
	from
		im_costs c
	where	
		cost_type_id = [im_cost_type_timesheet]
		$costs_user_constraint
    "
	...
	select
		...
		c.amount,
		c.currency,
		(now()::date)::varchar || ' ' || im_name_from_user_id(u.user_id) as cost_name
	from
		cc_users u
	...
	where
		h.user_id = u.user_id
		h.day = c.effective_date
		and h.project_id = c.project_id
		and h.user_id = c.cause_object_id
    "

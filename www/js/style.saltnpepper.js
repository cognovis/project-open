...
<div id="sideBar">
     <a href="#" id="sideBarTab"><img src="images/slide-button.gif"
		       ...
		       sideBarSlide(0, height, 0, width);
				       sideBarOpacity(0, 1);
							 isExtended = 1;
								    // move
	 main part
	      jQuery(".fullwidth-list").animate({marginLeft: "234px"},
	 slideDuration );
...
function sideBarSlide(fromHeight, toHeight, fromWidth, toWidth) {
	  ...
	  // $("sideBarContents").css ({'height': fromHeight, 'width':
	  fromWidth});
	  // $("#sideBarContents").animate( { 'height': toHeight, 'width':
	  toWidth }, { 'queue': false, 'duration': slideDuration }, "linear"
	  );
	  $("sidbar").css ({'height': fromHeight, 'width': fromWidth});
	  $("#sidebar").animate( { 'height': toHeight, 'width': toWidth }, {
...
$(function(){
	// Document is ready
	/* $('#sideBarTab').click( function() { extendContract(); return
	false; }); */
	/* $('#sideBarTab').click( function() { extendContract(); return
	false; }); */
});
...
jQuery().ready(function(){
    ...
    jQuery(".filter > .filter-block:first").prepend('<div
    class="filter-button"></div>');
...
jQuery().ready(function(){
	...
	document.getElementById("monitor_frame").insertBefore(node_to_move,
	node_insert_after.nextSibling);
	alert("here")
});

# /packages/flexbase/www/layout-adp.tcl
ad_page_contract {
...
set adp_dir "[acs_package_root_dir flexbase]/resources/forms/"
...
element::create layout-adp adp_file -label "[_ flexbase.Adp_file]" -datatype text -widget text \
	-html {size 20 maxlength 400}
element::create layout-adp action -datatype text -widget hidden
element::create layout-adp content -label "[_ flexbase.Content]" -datatype text -widget textarea \
	-html {rows 15 cols 100} -nospell

if { [form is_request layout-adp] } {
    set file_name [db_string get_filename {
	select adp_file from FLEXBASE_LAYOUT_PAGES 
	...
	set action update
		...
		-help_text "[_ flexbase.This_file_is_located_in] '${adp_dir}'" -value $file_name \
		-mode view 
    } else {
	set action insert
	# read default file
	...
	element::set_properties layout-adp adp_file \
		-help_text "[_ flexbase.This_file_will_be_placed_in] '${adp_dir}'"
    }
...
if { [info exists action] && $action == "update" } {
    set title "[_ flexbase.Edit_the_form_adp_file]"
} else {
    set title "[_ flexbase.Create_a_new_adp_file_for_this_form]"
}
...
if { [form is_valid layout-adp] } {
	...
	element::set_error layout-adp adp_file "[_ flexbase.This_file_already_exists]"
	return
    }
    ...
    # We can not check this because now the user can add core (not flexbase) attributes and 
    # there is no easy way to know wich ones are the core attributes in a particular page
    #if { $error_ids } {
	#element::set_error layout-adp content "[_ flexbase.Revise_your_attributes_id] 

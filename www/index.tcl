...
set profiles_without_category [db_list_of_lists pwt "
	...
	where	group_id = profile_id and 
		not exists (
		    select category_id 
		    from im_categories 
		    where	category = group_id::varchar
...
set profile_categories_without_group [db_list_of_lists pcwg "
	...
	where	category_type = 'Intranet Groups' and
		not exists (
		    	select	1
			from 	groups
			where	group_id::varchar = category
		)
"]
...
foreach otype {im_company im_office person} {
    set exists_p [ams::list::exists_p -package_key "contacts" -object_type $otype -list_name $otype]
    if {!$exists_p} {
...
set valid_page_sizes [list 25 50 100 500 1000 2000 10000]

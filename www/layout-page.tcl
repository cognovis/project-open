...
set type_list [list [list "[_ flexbase.absolute]" "absolute"] [list "[_ flexbase.relative]" "relative"] [list "[_ flexbase.adp]" "adp"] ]
...
if { [exists_and_not_null page_url] } {
    ...
    element::create page_layout page_url -datatype text -label "Page url" -html {size 50 maxlength 500} \
	    -help_text [_ flexbase.Use_a_relative_url_as_example]
    element::create page_layout layout_type -datatype text -label "Layout type" -widget select -options $type_list \
...
if { [form is_valid page_layout] } {
	...
	db_transaction {	
	    db_dml insert_page {
		insert into flexbase_layout_pages
	    ...
	    ns_log warning "\[TCL\]flexbase/www/layout-page --------> $errmsg"
	    ad_script_abort
	}
    } else {
	db_dml update_page {
	    update flexbase_layout_pages set table_width = :table_width

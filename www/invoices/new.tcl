...
-- Status change dates
--
	(select group_id, min(audit_date) as when from im_projects_status_audit
	group by group_id) s_create,
	(select min(audit_date) as when, group_id from im_projects_status_audit
	where project_status_id=74 group by group_id) s_quote,
	(select min(audit_date) as when, group_id from im_projects_status_audit
	where project_status_id=76 group by group_id) s_open,
	(select min(audit_date) as when, group_id from im_projects_status_audit
	where project_status_id=78 group by group_id) s_deliver,
	(select min(audit_date) as when, group_id from im_projects_status_audit
	where project_status_id=79 group by group_id) s_invoice,
	(select min(audit_date) as when, group_id from im_projects_status_audit
	where project_status_id in (77,81,82) group by group_id) s_close
...
<table><tr>
  ...
  <td>
	<table><tr>
	  <td>
	    <blockquote>
		To create a new invoice please select one or more projects<br>
...
if {$ctr==$how_many && $total_in_limited > 0 && $end_idx < $total_in_limited} {
    set next_start_idx [expr $end_idx + 1]
    set next_page "<a href=new?start_idx=$next_start_idx&[export_ns_set_vars url [list start_idx]]>Next Page</a>"
} else {
    set next_page ""
}
...
if { $start_idx > 1 } {
    ...
    set previous_page "<a href=new?start_idx=$previous_start_idx&[export_ns_set_vars url [list start_idx]]>Previous Page</a>"
} else {
    set previous_page ""
}
...
set submit_button "
      <tr>
	   ...
	   Invoice Currency: [im_currency_select invoice_currency "EUR"]
	   <input type=submit value='Select Projects for Invoice'> 
        </td>
      </tr>
"

...
ad_proc im_dynfield::generic_sql_option_list {
    {-widget_name:required}
} {
    Return the value of a sql from
} {
    set parameters [lindex [db_string parameters "select parameters from im_dynfield_widgets where widget_name = :widget_name"] 0]
    set custom_pos [lsearch $parameters custom]
    if { $custom_pos >= 0 } {
    	set custom_statement [lindex $parameters [expr $custom_pos + 1]]
    } else {
	return "Generic SQL Widget: Error: Didn't find 'custom' parameter"
    }
    set sql_pos [lsearch $custom_statement sql]
    if { $sql_pos >= 0 } {
    	set sql_statement [lindex $custom_statement [expr $sql_pos + 1]]
    } else {
	return "Generic SQL Widget: Error: Didn't find 'sql' parameter"
    }
    ...
    set memoize_max_age [parameter::get_from_package_key -package_key intranet-dynfield -parameter GenericSQLWidgetMemoizeMaxAgeDefault -default 600]
    set key_value_list [list]    
    return [util_memoize [list db_list_of_lists sql_statement $sql_statement] $memoize_max_age]
}

ad_proc im_dynfield::generic_sql_option_name {
    {-widget_name:required}
    {-object_id:required}
} {

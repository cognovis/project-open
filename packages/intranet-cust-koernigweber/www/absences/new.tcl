...
set form_fields {
	...
	{absence_type_id:text(im_category_tree) {label "[_ intranet-timesheet2.Type]"} {custom {category_type "Intranet Absence Type"}}}
    	{absence_day_p:boolean(radio) {label "[_ intranet-cust-koernigweber.AbsenceTimeframe]"} {options {"[_ intranet-cust-koernigweber.Day]" "[_ intranet-cust-koernigweber.Hour]" }}}
...
ad_form -extend -name absence -form {
    {start_date:date(date) {label "<span id=''absence_start_date_label>[_ intranet-timesheet2.Start_Date]</span>"} {before_html "<div id=absence_start_date_widget>" } {after_html "</div>"} }
    ...
    {duration_hours:float(text) {label "[lang::message::lookup {} intranet-timesheet2.Duration_hours {Duration (Hours)}]"} {help_text "[lang::message::lookup {} intranet-timesheet2.Duration_hours_help {Please specify the absence duration as a number of hours or a fraction of hours. Example: '1.50'=one hour,30 min}]"}}
...
ad_form -extend -name absence -on_request {
    ...
    if {![info exists duration_hours]} { set duration_hours "" }

...
for { set current_date $first_julian_date} { $current_date <= $last_julian_date } { incr current_date } {
    ...
    if { [info exists users_hours($current_date)] && ![empty_string_p $users_hours($current_date)] } {
 	set hours "$users_hours($current_date) [_ intranet-timesheet2.hours]"
	...
	if { [info exists users_hours($current_date)] } {
		if { [info exists unconfirmed_hours($current_date)] } { 
		    ...
		    if { 0 == $unconfirmed_hours($current_date) } { 
			# ns_log notice "KHD: [info exists hash_conf_object_id($julian_date)]"	
			...
			set html "$html<br>Freizugeben: <span id='hours_confirmed_red'>$unconfirmed_hours($current_date)&nbsp;[_ intranet-timesheet2.hours]</span>"
...
# Show confirmation link only if there are hours to confirm 

set ul_links ""

# construct search_date
set search_month [expr [lsearch [long_month_list] $month]+1]  
if { $search_month < 10 } {set search_month "0$search_month"}
set search_date "$year-$search_month"

set hours_to_confirm_sql "
	select  count(*)
        from    im_hours
        where   conf_object_id is null
        and day like '%$search_date%'
        and user_id = :current_user_id
"

set hours_to_confirm_p [db_string hours_to_confirm_p $hours_to_confirm_sql -default "0"]
...
if { $hours_to_confirm_p } { 
	...
	append ul_links "Alle Stunden sind freigegeben"
}

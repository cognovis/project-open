...
ad_form -name register -export {next_url user_id return_url} -form {
	...
	{label "Abteilung"}
	{options {[im_cost_center_options]}}
    }
}


ad_form -extend -name register  -form [auth::get_registration_form_elements]  -validate {
    {email
...
ad_form -extend -name register -on_request {
    ...
    group::add_member -user_id $user_id -group_id 463 -no_perm_check
    db_dml insert "insert into im_employees (employee_id,department_id) values (:user_id,:department_id)"

    # Change the authority
    db_dml update_authority "update users set authority_id = 33597 where user_id = :user_id"

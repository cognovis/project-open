...
-- @cvs-id $Id: contacts-messages-create.sql,v 1.2 2009/02/09 02:40:21 cvs Exp $
--
--
...
create table contact_message_items (
	item_id                 integer
                                constraint contact_message_items_id_fk 
				references cr_items(item_id)
                                constraint contact_message_items_id_pk primary key,
        owner_id                integer
                                constraint contact_message_items_owner_id_fk 
				references acs_objects(object_id) on delete cascade
                                constraint contact_message_items_owner_id_nn not null,
        message_type            varchar(20)
                                constraint contact_message_items_message_type_fk 
				references contact_message_types(message_type)
                                constraint contact_message_items_message_type_nn not null,
	locale			varchar(30)
				constraint contact_message_items_locale_fk 
...
create table contact_message_log (
        message_id              integer
				...
				constraint contact_message_log_message_id_fk 
				references acs_objects(object_id),
        message_type            varchar(20)
                                constraint contact_message_log_message_type_fk 
				references contact_message_types(message_type)
                                constraint contact_message_log_message_type_nn not null,
        sender_id               integer
                                constraint contact_message_sender_id_fk 
				references users(user_id)
                                constraint contact_message_sender_id_nn not null,
        recipient_id            integer
                                constraint contact_message_recipient_id_fk 
	...
	description             text,
        ...
        content_format          varchar(200)
                                constraint contact_message_log_content_format_fk 

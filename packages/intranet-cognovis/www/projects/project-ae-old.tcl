...
if {$enable_nested_projects_p} {
    ...
    if {[info exists project_id]} { set ::super_project_id $project_id } else { set ::super_project_id 0 }
...
if {[form is_request $form_id]} {
	...
	if { [exists_and_not_null start_date] } { set start_date $todays_date }
	if { [exists_and_not_null end_date] } { set end_date $todays_date }
	if { [exists_and_not_null end_time] } { set end_time "12:00" }
	set button_text "[_ intranet-core.Save_Changes]"
    ...
    if {[exists_and_not_null on_track_status_id]} {
	set on_track_status_id [im_project_on_track_status_green]
    }
    
    if {[exists_and_not_null percent_completed]} {
	set percent_completed 0
    }

    if {[exists_and_not_null project_budget_currency]} {
...
if {[form is_valid $form_id]} {
	...
	set end_date [template::util::date get_property sql_timestamp $end_date]
	...
	if {[info exists previous_project_type_id]} {
		...
		if {[exists_and_not_null field_cnt]} {

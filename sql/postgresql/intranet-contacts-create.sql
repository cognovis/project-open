...
(    select rel_type,
       ...
       from acs_rel_types
      where rel_type in ( select object_type from acs_object_types where supertype in ('contact_rel','im_biz_object_member'))
)
UNION
(    select rel_type,
...
alter table users_contact add constraint users_contact_user_id_fk foreign key (user_id) references persons(person_id);

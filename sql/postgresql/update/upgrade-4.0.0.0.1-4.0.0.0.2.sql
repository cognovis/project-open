...
SELECT acs_log__debug('/packages/intranet-timesheet2-workflow/sql/postgresql/upgrade/upgrade-4.0.0.0.1-4.0.0.0.2.sql','');

CREATE OR REPLACE FUNCTION im_ts_notify_applicant_not_approved_custom_kw(integer, character varying, character varying)
  RETURNS integer AS
$BODY$
declare
	...
	p_custom_arg            alias for $3;
        ...
        v_task_owner            integer;	v_note			varchar;
	...
	v_project_manager_label	text;		v_note_label		text;
...
begin
	...
	v_note_label := acs_lang_lookup_message(v_locale, 'intranet-core', 'Note');	

...
'"$indent_html<a href=/intranet-timesheet2-tasks/new?[export_url_vars project_id task_id return_url]>
$task_nr</a>"','','',0,'');
insert into im_view_columns (column_id, view_id, group_id, column_name, column_render_tcl,
...
'"[if {[string equal t $red_p]} { set t "<font color=red>[string range $end_date 0 9]</font>" } else { set t [string range $end_date 0 9] }]"','(child.end_date < now() and coalesce(child.percent_completed,0) < 100) as red_p','',8,'');
insert into im_view_columns (column_id, view_id, group_id, column_name, column_render_tcl,
...
'"[if {[string equal t $red_p]} { set t "<font color=red>[string range $end_date 0 9]</font>" } else { set t [string range $end_date 0 9] }]"','(child.end_date < now() and coalesce(child.percent_completed,0) < 100) as red_p','',3,'');
insert into im_view_columns (column_id, view_id, group_id, column_name, column_render_tcl,
